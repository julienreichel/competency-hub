...
describe('User Model', () => {
  describe('static mapping and utilities', () => {
    ...
    it('normaliseStudentProgress handles array, {items}, null, and invalid', () => {
      // Array
      const arr = [
        {
          id: 'sp1',
      ...
      const resultArr = (
        User as unknown as { normaliseStudentProgress: (entries: unknown) => unknown[] }
      ).normaliseStudentProgress(arr);
      expect(resultArr.length).toBe(1);
      expect((resultArr[0] as { id: string }).id).toBe('sp1');
      // { items: [...] }
      const itemsObj = { items: arr };
      const resultItems = (
        User as unknown as { normaliseStudentProgress: (entries: unknown) => unknown[] }
      ).normaliseStudentProgress(itemsObj);
      expect(resultItems.length).toBe(1);
      expect((resultItems[0] as { id: string }).id).toBe('sp1');
      // null
      expect(
        (
          User as unknown as { normaliseStudentProgress: (entries: unknown) => unknown[] }
        ).normaliseStudentProgress(null),
      ).toEqual([]);
      // invalid
      expect(
        (
          User as unknown as { normaliseStudentProgress: (entries: unknown) => unknown[] }
        ).normaliseStudentProgress({}),
      ).toEqual([]);
    });

    it('normaliseEvaluationAttempts handles array, {items}, null, and invalid', () => {
      const arr = [
        {
          id: 'ea1',
          studentId: 'u1',
      ...
      const resultArr = (
        User as unknown as { normaliseEvaluationAttempts: (entries: unknown) => unknown[] }
      ).normaliseEvaluationAttempts(arr);
      expect(resultArr.length).toBe(1);
      expect((resultArr[0] as { id: string }).id).toBe('ea1');
      const itemsObj = { items: arr };
      const resultItems = (
        User as unknown as { normaliseEvaluationAttempts: (entries: unknown) => unknown[] }
      ).normaliseEvaluationAttempts(itemsObj);
      expect(resultItems.length).toBe(1);
      expect((resultItems[0] as { id: string }).id).toBe('ea1');
      expect(
        (
          User as unknown as { normaliseEvaluationAttempts: (entries: unknown) => unknown[] }
        ).normaliseEvaluationAttempts(null),
      ).toEqual([]);
      expect(
        (
          User as unknown as { normaliseEvaluationAttempts: (entries: unknown) => unknown[] }
        ).normaliseEvaluationAttempts({}),
      ).toEqual([]);
    });
