...
- **Attributes:** student, period (startDate, endDate), summary (domains covered, competencies advanced/acquired, total activity), domain progress (overallProgress, progressDelta, itemsChanged), generatedAt, generatedBy
- **Relations:**
  - Generated by Educator or Parent
  - Linked to Student (via studentId)
  - Aggregates StudentSubCompetencyProgress data
  - Groups by Domain → Competency → SubCompetency hierarchy
- **Features:**
  - Period-based filtering for progress tracking
  - Domain-level progress summaries with color coding
  - Print-optimized layouts (A4, hide navigation, page breaks)
